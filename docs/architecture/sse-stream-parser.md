...
> **注意**: `storeService` 没有 `_` 前缀，可能是公共属性。注入方式推断为构造函数参数注入。
...
|--------|------|------|---------|-------------|
| knowledge 分支自动确认 | ~7502574 | 确认弹窗 | `confirm_status==="unconfirmed" && auto_confirm` | ✅ auto-confirm-commands v3 |
| else 分支自动确认 | ~7503319 | 确认弹窗 | `confirm_status!=="confirmed"` | ✅ service-layer-runcommand-confirm v6 |
| 可继续错误判断 | ~8696378 | 错误阻断 | J 变量不包含错误码 | ✅ bypass-loop-detection |
| 可恢复错误列表 | ~8695303 | 错误阻断 | efh 列表不包含错误码 | ✅ efh-resume-list |
| Alert "继续"按钮 | ~8702342 | UI限制 | 需要手动点击 | ✅ auto-continue-thinking |
