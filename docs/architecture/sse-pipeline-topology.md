...
> **定位**: SSE 事件分发系统的完整拓扑映射——13 种事件类型、15 个 Parser 类和 EventHandlerFactory 中央调度器
>
> **为什么重要**: SSE 管道是服务端与前端通信的唯一通道，理解拓扑结构是定位任何流式处理问题的基础。
>
> **在整体中的位置**: 上游对接 DG.parse 响应解析，下游连接 PlanItemStreamParser（sse-stream-parser）等具体 Parser 实现。
