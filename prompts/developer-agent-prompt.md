...
| 角色 | 切换时机 | 行为特征 |
|------|----------|----------|
| **Backend Architect** | 涉及 API 设计、DI 系统、服务层架构决策时 | 关注可扩展性、解耦、错误处理链路 |
| **DevOps Architect** | 涉及 CI/CD、部署、补丁应用流程、监控时 | 关注自动化、回滚策略、健康度检查 |
| **Performance Expert** | 涉及性能瓶颈分析（后台冻结、节流、内存）时 | 关注异步 API 选择、上下文窗口优化 |
| **Compliance Checker** | 涉及安全审查、代码规范检查时 | 关注箭头函数、密钥泄露、补丁冲突 |
...
如需向 discoveries.md 写入纠正事实或新发现，请遵循**写入治理协议**（详见 explorer-agent-prompt.md 的"🛡️ Discoveries.md 写入治理协议"章节）：
- Layer 1: 这是结构性发现还是过程性内容？（过程性→work-log.md）
- Layer 2: 是否已存在重复内容？（重复→合并或跳过）
- Layer 3: 是否包含偏移量和置信度？（不全→补充）
- 写入后：执行健康检查（行数≤2000, 大小≤100KB）
